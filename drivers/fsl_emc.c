...
void EMC_Init(EMC_Type *base, emc_basic_config_t *config)
{
    ...
    SYSCON->EMCSYSCTRL = SYSCON_EMCSYSCTRL_EMCSC(0);//   SYSCON->EMCSYSCTRL = SYSCON_EMCSYSCTRL_EMCSC(1);

    //SYSCON->EMCSYSCTRL = SYSCON->EMCSYSCTRL | 1;

 //   SYSCON->EMCDLYCTRL = SYSCON->EMCDLYCTRL & 0x1F00; // Added by Nandaraj
 //   SYSCON->EMCDLYCTRL = SYSCON->EMCDLYCTRL | 0x05; // Added by Nandaraj
 //   SYSCON->EMCDLYCTRL = 0x1F08;
    /* Set the endian mode. */
                        ...
                        uint32_t totalChips)
{
    ...
    for (count = 0; (count < totalChips); count++)
    {
        ...
        else
        {
            base->DYNAMIC[dynamicConfig->chipIndex].DYNAMICCONFIG =
                EMC_DYNAMIC_DYNAMICCONFIG_MD(dynamicConfig->dynamicDevice) | EMC_ADDRMAP(dynamicConfig->devAddrMap) & 0xFFF7FFFF;
    ...
    base->DYNAMICDAL        = 5;//EMC_CalculateTimerCycles(base, timing->tDal_Ns, 0) & EMC_DYNAMICDAL_TDAL_MASK;
    base->DYNAMICWR         = 3;//EMC_CalculateTimerCycles(base, timing->tWr_Ns, 1) & EMC_DYNAMICWR_TWR_MASK;
    base->DYNAMICRC         = 3;//EMC_CalculateTimerCycles(base, timing->tRc_Ns, 1) & EMC_DYNAMICRC_TRC_MASK;
    ...
    base->DYNAMICRRD        = 3;//EMC_CalculateTimerCycles(base, timing->tRrd_Ns, 1) & EMC_DYNAMICRRD_TRRD_MASK;
    ...
    for (count = 0; (count < totalChips); count++)
    {
        ...
        {
            base->DYNAMIC[dynamicConfig->chipIndex].DYNAMICCONFIG |= EMC_DYNAMIC_DYNAMICCONFIG_B_MASK | 0x80000 ;
            dynamicConfig++;
        }
    }
}
